...
newtype Store = Store (ForeignPtr (Ptr EsStore))
...
openCompatibleStore options = mask $ \restore -> do
  ...
  case opened of
    ...
    Right () -> do
      ...
      pure (Right (Store owner))


-- | Hands the owner cell to an engine call for as long as the call runs.
--
-- No Haskell-side lock guards the cell, so calls on one store run
-- concurrently and the pool size and runtime threads the store was opened
-- with mean what they say. The engine owns the linearization: it leases the
-- store under its registry lock, nulls the cell there when a close begins,
-- validates the cell against that registry before touching the store, and
-- drains the calls already in flight before destroying anything. A close
-- racing a call therefore loses to a closed-store error rather than to a
-- freed store. The null check here only saves a foreign call; the engine
-- repeats it authoritatively. Holding the cell through 'withForeignPtr' keeps
-- the finalizer that frees it from running underneath the call.
--
-- That null check is deliberately outside the engine's registry lock, so it is
-- an unsynchronized read of a word the engine writes while holding that lock.
-- It is sound only because the cell is a single pointer-sized word that
-- transitions once, from non-null to null: it cannot tear, and a stale
-- non-null costs one foreign call the engine then rejects. Widening the cell
-- to carry anything more -- a generation counter, a flags word, a second
-- field -- retires that argument, and the read must then move under the
-- engine's lock or go away.
withOpenStore
  :: Store
  -> (Ptr (Ptr EsStore) -> IO (Either EngineError value))
  -> IO (Either EngineError value)
withOpenStore (Store owner) action =
  withForeignPtr owner $ \cell -> do
    handle <- peek cell
    if handle == nullPtr
