...
import EventSorcery.Job (
  ...
  JobKind (JobKind),
  JobLeaseResult (LeaseHeld, LeaseLost),
  JobSeed (JobSeed),
  JobSettlement (SettlementApplied, SettlementFenced),
  JobSettlementToken,
...
exerciseAtomicCommit store = do
  ...
  conflicted <-
    commitWithJob
      ...
      stream
      0
      (ProposedEvent "reopened" "1" payload :| [])
      conflictSeed
  versionAfterConflict <- currentVersion store stream
  jobsAfterConflict <- pollJobs store kind now (PollLimit 10)

  case conflicted of
    Left (OptimisticConflict _)
      | versionAfterConflict == Right 1 && jobsAfterConflict == Right [] -> pure ()
    _ -> error "failed atomic commit left an event or job behind"
  where
...
exerciseLeaseFencing :: Store -> IO ()
exerciseLeaseFencing store = do
  enqueued <- enqueueJob store seed
  expectUnit "lease-fencing job enqueue failed" enqueued

  (firstDetails, staleToken) <- claimToken store identifier now
  (_, currentToken) <- claimToken store identifier later
  renewed <- renewJob store firstDetails.reference (JobInstant 120_000)
  staleSettlement <- acknowledgeJob store staleToken
  currentSettlement <- acknowledgeJob store currentToken

  if renewed == Right LeaseLost
    && staleSettlement == Right SettlementFenced
    && currentSettlement == Right SettlementApplied
    then pure ()
    else error "stale job ownership was not fenced after lease loss"
  where
    identifier = JobId "01ARZ3NDEKTSV4RRFFQ69G5FAZ"
    seed = JobSeed identifier kind payload now
